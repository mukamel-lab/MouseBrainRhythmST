...
gene_df=gene_df %>% pivot_wider(id_cols = c(gene, cluster),
  ...
  dplyr::filter(sig!="None", (rel_amp_wt>0.02 | rel_amp_app>0.02))

gene_df=split(gene_df, gene_df$cluster)

gene_df=gene_df$`Dentate Gyrus-sg`
...
mean_expr2=gene_df2[,c("gene","baseMean_app", "baseMean_wt")] %>%
  pivot_longer(c("baseMean_app", "baseMean_wt"), names_to = "geno",
                                      values_to ="expr") %>%
...
row_ha2 = rowAnnotation(
  ...
  basemean=anno_points(as.matrix(mean_expr[,3:4]),
