...
deseqs_old=readRDS("~/desp1/precast/prec_c25q25g3000/deseq_geno_age_int/age_geno_wald_full/deseqs.rds")
...
ggplot(res %>% na.omit() %>% arrange(-padj))+
  ...
  scale_size_manual(values=c(.1,1))+theme_bw() +xlab("Log2 Normalized Expression") +
  ylab("Effect Size: Genotype + Genotype:Age")+
  guides(size="none")
...
ggplot(res %>% na.omit() %>% arrange(-padj))+
  ...
  xlab("Effect Size: Genotype + Genotype:Age")+
