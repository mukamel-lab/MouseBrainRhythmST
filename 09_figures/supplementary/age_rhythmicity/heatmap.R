...
names(deseqs)=gsub("Olfactory Tubercle", "Amygdala", names(deseqs))
...
sig_genes=res_sig %>% mutate(region=plyr::mapvalues(cluster, names(cluster_color), regions))

sig_genes=split(sig_genes, sig_genes$region)

cnts=lapply(names(deseqs), function(df) {
  temp_cnts=counts(deseqs[[df]], normalized=T) %>% 
    as.data.frame() %>% 
    rownames_to_column("gene")
  glist=plyr::mapvalues(df, names(cluster_color), regions)
    temp_cnts %>% filter(gene %in% sig_genes[[glist]]$gene)
})
names(cnts)=names(deseqs)
...
g_to_mark=c("Cst7","C4b" , "Gfap","Avp","Oxt")
