...
counts=readRDS("analysis/deseq_rhyth/cortex_regional/ncounts_NTG.rds")

coefs=readRDS("analysis/deseq_rhyth/cortex_regional/res_NTG.rds") %>%
...
interact_res = readRDS("analysis/deseq_region_interaction/cortex_regional_NTG_reg_int.rds") %>% 
 dplyr::select(gene,age,comparison, padj)%>%
 ...
 to_plt=counts %>% dplyr::filter(gene==gene_to_plot) 
  ...
  crv = crv %>%
    mutate(
      ...
      region = recode(region,
           ...
           region=factor(region,levels=c("Rostral","Medial", "Caudal")))
  ...
  to_plt= to_plt%>%
    ...
    reg_col=ggsci::pal_d3()(3) %>% c("Rostral","Medial", "Caudal")
  ...
  gtp=res_sig %>% dplyr::filter(padj_C<0.05, padj_R>.1, padj_M>0.1, (C_R <0.1| C_M<0.1))
  
  plts=lapply(c("Elk1", "Grasp", "Sertad1", "Rasl10a" ), make_plot)
  
  
  for(p in plts) {plot(p); readline(" ")}

  pdf("figures/figure5/plots/sin_plots_caud_spec_RMC.pdf", height=3, width = 1.8)
