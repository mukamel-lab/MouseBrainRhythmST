...
interact_res = readRDS("analysis/deseq_region_interaction/cortex_regional_NTG_reg_int.rds") %>% 
 ...
 to_plt=counts %>% dplyr::filter(gene==gene_to_plot) 
  ...
  gtp=interact_res %>% dplyr::filter(padj<0.05)
  plts=lapply(clock_g[clock_g %in% gtp$gene], make_plot)
  
  
  

  pdf("figures/figure5//sin_plots_sig_clock_RMC.pdf", height=3, width = 1.8)
