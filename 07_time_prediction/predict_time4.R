...
##############################################
################################################
res_sig = readRDS("deseq_rhyth2/APP_harmonic/par_im/res_sig.rds") %>%
  arrange(padj)

# res_sig = readRDS("deseq_rhyth2/both_no_geno_term/par_im/res_sig.rds") %>% dplyr::filter(padj<0.05) %>%
#   arrange(padj)
...
predict_wt=pbmclapply(names(agg_c_wt), function(clst) {
  ...
  z_c=predict_app[[clst]][["z_c"]]
  
  
 
  
  z_s=predict_app[[clst]][["z_s"]]
...
ggplot(both %>% dplyr::filter(cluster %in% levels(cluster)[1:11]),
  ...
  theme(legend.position = "bottom")+ggtitle("Train WT, Predict APP23")
