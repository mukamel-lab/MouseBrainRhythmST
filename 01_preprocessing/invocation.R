...
q_val=25
n_genes=3000
min_features="5_nofilt"

#####################

prec_run=glue("prec_c{k_val}q{q_val}g{n_genes}")
dir.create(prec_run)

####################################
# copy invocation to dir
####################################
system(glue("cp ~/desp1/precast/prec_pipe_short.R {prec_run}/invocation.R"))

####################################
# copy annotation template to dir
####################################
...
prec=readRDS("/home/agelber/desp1/precast/prec_c22q22g3000_nofilt/prec.rds")

prec = AddAdjList(prec, platform = "Visium")


prec = AddParSetting(prec, 
                     Sigma_equal = FALSE, 
                     verbose = TRUE, 
                     coreNum=15,
                     int.model="EEE")

saveRDS(prec, glue("/home/AD/agelber/desp1/precast/{prec_run}/prec.rds"),
        compress = F)
...
prec@seulist=lapply(prec@seulist, function(x) {
  names(x@assays)="RNA"
  DefaultAssay(x)="RNA"
  x
})
...
seuInt = IntegrateSpaData(prec, species = "Mouse")

seuInt$sample=plyr::mapvalues(as.integer(seuInt$batch), from = 1:65,
                              to=metadata$sample)



saveRDS(seuInt, glue("/home/AD/agelber/desp1/precast/{prec_run}/seuInt.rds"),
        compress = F)

seuInt=RunUMAP(seuInt, reduction = "PRECAST",
...
p = SpaPlot(seuInt, batch = NULL, point_size = 1.4,
            cols = cols_cluster, combine = F)
p=lapply(1:65, function(pl){p[[pl]]+coord_fixed()+
    theme(legend.position = "none")+ggtitle(metadata$sample[pl])})
...
for(cl in unique(seuInt$cluster)){
  ...
  p_clust = SpaPlot(seuInt, batch = samps, point_size = 1,
