...
set.seed(1)
shuffle_list=lapply(1:100, function(a) {sample.int(16, 16, replace = F)})
...
run_one_shuffle= function(i) {
  ...
  balanced$sex=balanced$sex[shuffle_list[[i]]]
  ...
  "sex_int_geno_spec/shuffle_q_value/young/res{i}.rds"))
  invisible(NULL)
}
pbmclapply(1:100, run_one_shuffle, mc.cores = 6, mc.allow.recursive = T, ignore.interactive = T)