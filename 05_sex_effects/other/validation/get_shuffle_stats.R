...
res_mf_rats=pbmclapply(list.files("shuffle_num_rg/shuffles/", full=T), 
                     readRDS) %>% bind_rows(.id="seed") %>% mutate(shuf="shuffled")


true_MF=readRDS("shuffle_num_rg/true_MF.rds") %>%  
...
all=bind_rows(list(res_mf_rats, true_MF %>% mutate(shuf="unshuffled")))
...
p <- ggplot(all , aes(x = cluster, y = n1_rat, col = shuf)) +
  ...
  ylab("Log2(Num. rhythmic genes in female/male)") +
...
true_MF_rat2=
  ...
  pivot_wider(id_cols = c("cluster"), names_from = "sex", 
              values_from = starts_with("n")) %>%
  mutate(true_n1_rat=n1_Female/n1_Male,
         true_n05_rat=n05_Female/n05_Male, 
         true_n01_rat=n01_Female/n01_Male)


emp_p=left_join(res_mf_rats,true_MF_rat2, by="cluster") %>% group_by(cluster) %>% 
  summarise(emp_p=(sum(abs(n1_rat) >= abs(true_n1_rat)) + 1) / (n() + 1),
            .groups = "drop"
  ) %>%
