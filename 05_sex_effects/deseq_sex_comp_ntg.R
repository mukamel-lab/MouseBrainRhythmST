...
res_f_m = pbmclapply(names(agg_exp), function(cl_n) {
  ...
  try({
    suppressWarnings({
      ...
      res_m = results(dds_m, tidy = T) %>% dplyr::rename(gene = row) %>% 
        mutate(sex ="M")
...
saveRDS(coefs, "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/sex_int_geno_spec/coefs_wt.rds")

saveRDS(ncounts, "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/sex_int_geno_spec/counts_wt.rds")

saveRDS(res_f_m %>% bind_rows(.id="cluster"),"~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/sex_int_geno_spec/res_wt_mf.rds")
