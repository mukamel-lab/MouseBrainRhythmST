...
run_one_shuffle= function(i) {
  ...
  rgs=readRDS("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/sex_int_geno_spec/shuffle_mean_amp/rg_MF.rds")
  ...
  deseqs=pbmclapply(names(agg_exp) %>% setNames(.,.), function(cl){
    ...
    try({
      suppressWarnings({
        ...
        dds_y=deseq[ rgs[[cl]]$rgs,deseq$sex=="F"]
        dds_y= dds_y %>%
          DESeq(
            test = "LRT",
            reduced = ~ age,
        ...
        coefs_y=coefficients(dds_y) %>% as.data.frame() %>% rownames_to_column("gene") %>%
          mutate(amp=sqrt(t_s ^ 2 + t_c ^ 2), sex="F") %>%
          group_by(sex) %>% summarise(amp=mean(amp))
        
        dds_o=deseq[ rgs[[cl]]$rgs,deseq$sex=="M"]
        dds_o=dds_o %>% 
          DESeq(
            test = "LRT",
            reduced = ~ age,
        ...
        coefs_o=coefficients(dds_o) %>% as.data.frame() %>% rownames_to_column("gene") %>%
          mutate(amp=sqrt(t_s ^ 2 + t_c ^ 2), sex="M") %>%
          group_by(sex) %>% summarise(amp=mean(amp))
  ...
  saveRDS(res, glue("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/",
                    "sex_int_geno_spec/shuffle_mean_amp/shuffles/res{i}.rds"))
  invisible(NULL)
}
lapply(401:1000, run_one_shuffle)#, mc.cores = 4, mc.allow.recursive = T)