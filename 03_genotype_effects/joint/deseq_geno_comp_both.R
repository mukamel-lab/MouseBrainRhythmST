  ...
  try({
    suppressWarnings({
      ...
      deseq = DESeqDataSetFromMatrix(
        countData = counts2[, c("gene", md$id)]  ,
        colData = md,
        design =  ~ genotype + sex +age + t_s + t_c + age:(t_s+t_c)+genotype:(t_s + t_c),
        tidy = T
      ) %>%
        estimateSizeFactors(type = "ratio") %>%
        ...
        DESeq(test = "LRT",
              reduced =  ~ genotype + sex + age + t_s + t_c + age:(t_s+t_c),
              fitType = "local")
      
      
      
      coefs = coefficients(deseq) %>% as.data.frame() %>% rownames_to_column("gene")
      coefs = coefs %>% mutate(app_o_amp = sqrt(t_s ^ 2 + t_c ^ 2), 
                               app_o_phi= atan2(t_s, t_c) %% (2 * pi),          
                               app_o_phi_hr=(12 / pi) * app_o_phi,
                               wt_o_amp = sqrt((t_s + genotypeWT.t_s) ^ 2 + (t_c + genotypeWT.t_c) ^ 2),
                               
                               wt_o_phi=atan2(t_s+ genotypeWT.t_s, t_c+ genotypeWT.t_c) %% (2 * pi),
                               wt_o_phi_hr=(12 / pi) * wt_o_phi,
                               
                               app_y_amp = sqrt((t_s+age7.months.t_s) ^ 2 + (t_c+age7.months.t_c) ^ 2), 
                               app_y_phi= atan2((t_s+age7.months.t_s), (t_c+age7.months.t_c)) %% (2 * pi),          
                               app_y_phi_hr=(12 / pi) * app_y_phi,
                               wt_y_amp = sqrt((t_s + age7.months.t_s+genotypeWT.t_s) ^ 2 + (t_c + age7.months.t_c+genotypeWT.t_c) ^ 2),
                               
                               wt_y_phi=atan2(t_s+ genotypeWT.t_s+age7.months.t_s, t_c+ genotypeWT.t_c+age7.months.t_c) %% (2 * pi),
                               wt_y_phi_hr=(12 / pi) * wt_y_phi,
                                 )
...
saveRDS(coefs, "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/joint/coefs_b.rds")

saveRDS(ncounts, "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/joint/counts_b.rds")

saveRDS(res_a_w %>% bind_rows(.id="cluster"),"~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/joint/res_b_aw.rds")
