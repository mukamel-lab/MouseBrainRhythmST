...
rg=readRDS("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/shuffle_amp/rg_young.rds")
...
for(i in 201:500){
    ...
    try({
      suppressWarnings({
        ...
        deseq = DESeqDataSetFromMatrix(
          ...
          colData = md,
          design =  ~ genotype + t_s + t_c+genotype:(t_c+t_s),
          tidy = T
        ) %>%
          estimateSizeFactors(type = "ratio") 
        deseq=DESeq(deseq[rg[[cl]],])
  ...
  saveRDS(res, glue("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/shuffle_amp/shuffle_young/res{i}.rds"))
