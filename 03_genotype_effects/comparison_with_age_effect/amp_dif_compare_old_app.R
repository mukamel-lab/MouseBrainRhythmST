...
res1=left_join(readRDS("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/old/coefs_o.rds"),
               readRDS("deseq_rhth_int2/geno_int_new/old/interaction_results_readjusted_o.rds") %>%
...
amp_ef_g=readRDS("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/comparison_with_age_effect/young_app_genes.rds")

res3=left_join(res3,amp_ef_g) 
  
  

res3=res3 %>% mutate(sig=
                       case_when(fdr.x<fdr_cut &fdr.y<fdr_cut ~"Both",
                                 fdr.x<fdr_cut & fdr.y>=fdr_cut ~"APP23 14 mo.",
                                 fdr.x>=fdr_cut & fdr.y<fdr_cut ~ "Age",
                                 sig=="Genotype" ~ "APP23 7 mo.",
                                 sig=="Both" ~ "Both", 
                                 .default = NA_character_)) %>%na.omit()

#saveRDS(res3 %>% dplyr::select(cluster, gene, sig), "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/comparison_with_age_effect/all_genes.rds")
...
p=ggplot(df3, aes(x = resid_age, y = resid_geno, colour = sig, group = sig)) +
  ...
  sm_statCorr( fullrange = T,
               linetype = "dashed",legends = T,text_size = 6*5/14
  ...
  coord_fixed()+ggsci::scale_color_bmj()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  scale_y_continuous(limits = c(NA, 1), oob=scales::squish)

saveRDS(p, "~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/comparison_with_age_effect/plot_amp_oldapp.rds")

p2=readRDS("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/comparison_with_age_effect/plot_amp_youngapp.rds")

pdf("~/desp1/precast/prec_c25q25g3000/deseq_rhth_int2/geno_int_new/figures/age_amp_com.pdf", height = 4, width = 7)
